...
Menu::Menu(sf::RenderWindow& window, sf::Music& music, sf::Texture& Background) : m_window(window), m_music(music), m_optionSelected(false), m_visible(true), m_startingScreenVisible(true), p_selectedOption(false), difficultySelected(false), scenerySelected(false), characterSelected(false) {
    ...
    m_gameName.setPosition(550, 100);
    ...
    m_pressAnyButton.setPosition(500, 850);
    createColorColumns();
    ...
    p_Guest.setFont(m_font);
    p_Guest.setString("Play as a Guest");
    p_Guest.setCharacterSize(30);
    p_Guest.setFillColor(sf::Color::White);
    p_Guest.setPosition(350, 300);

    p_CreateUser.setFont(m_font);
    p_CreateUser.setString("Create User");
    p_CreateUser.setCharacterSize(30);
    p_CreateUser.setFillColor(sf::Color::White);
    p_CreateUser.setPosition(350, 300);
    ...
    if (!movingImageTexture1.loadFromFile("PNG/Dead Baily.png")) {
        // Handle error
    }
    m_movingImagesTextures.push_back(movingImageTexture1);

    sf::Texture movingImageTexture2;
    if (!movingImageTexture2.loadFromFile("PNG/Cool Baily.png")) {
        // Handle error
    }
    m_movingImagesTextures.push_back(movingImageTexture2);

    sf::Texture movingImageTexture3;
    if (!movingImageTexture3.loadFromFile("PNG/Baily is winning.png")) {
        // Handle error
    }
...
void Menu::about() {
    ...
    if (!aboutTexture.loadFromFile("images/background.png")) {
        // Handle error
    }
    sf::Sprite aboutBackground(aboutTexture);

    sf::Text aboutText("About the Game", m_font, 50);
    aboutText.setPosition(650, 150);
    sf::Text aboutTex("The game is mainly about a dinosaur that tries to avoid hitting",m_font,30);
    aboutTex.setPosition(650, 250);
    sf::Text aboutTe("the obstacles that he faces during the way along the map that the ", m_font, 30);
    aboutTe.setPosition(650, 230);
    sf::Text aboutT("the obstacles that he faces during the way along the map that the ", m_font, 30);
    aboutT.setPosition(650, 210);
    while (true) {
        sf::Event event;
    ...
    while (m_window.pollEvent(event)) {
        ...
        if (event.type == sf::Event::Closed) {
            ...
            if (event.mouseButton.button == sf::Mouse::Left) {
                ...
                if (p_Guest.getGlobalBounds().contains(mousePosition.x, mousePosition.y)) 
                {
                    //start game
                }
                else if (p_CreateUser.getGlobalBounds().contains(mousePosition.x, mousePosition.y)) 
                {
            ...
            else if (event.key.code == sf::Keyboard::Up)
            {
                ...
                // Handle the selected option
                if (p_selectedOption == 0) {
                    p_optionSelected = true;
                }
                else if (p_selectedOption == 1)
                {
                    // Handle the second play option
                    p_optionSelected2 = true;
                }
...
bool Menu::UserSelected()
{
    return p_selectedOption;
}
